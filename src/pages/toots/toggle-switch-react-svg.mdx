...
Since most browsers use native OS inputs or draw their own custom controls, so
I started from scratch with an SVG.
...
start with a hidden `<input type="checkbox" />` control and use CSS to bind the

...
const getReflectedEndControl = (command) => {
  const endControl = command.controlPoints[1];
  const center = command.startEnd[1];
  const delta = [center[0] - endControl[0], center[1] - endControl[1]];
    ...
    case 'H': {
      switch (isAbsolute) {
        case true:
          return [
            prevEnd,
            [parameters[0], prevEnd[1]],
          ];
        default:
          return [
            prevEnd,
            [parameters[0] + prevEnd[0], prevEnd[1]],
          ];
      }
    }
    case 'V': {
      switch (isAbsolute) {
        case true:
          return [
            prevEnd,
            [prevEnd[0], parameters[0]],
          ];
        default:
          return [
            prevEnd,
            [prevEnd[0], parameters[1] + prevEnd[1]],
          ];
      }
    }
    case 'Z': {
    ...
    case 'Q': {
      ...
      switch (isAbsolute) {
        case true:
          return [first];
        default:
          return [[start[0] + first[0], start[1] + first[1]]];
      }
    }
    case 'T': {
      const first = [
...
const getIsolatedCommandString = ((command) => {
  ...
  switch (code) {
    ...
    case 'C':
    case 'S': {
      return `M${startEnd[0].join(',')} S${controlPoints[0].join(',')} ${controlPoints[1].join(',')} ${startEnd[1].join(',')}`;
    }
    case 'Q':
    case 'T': {
      return `M${startEnd[0].join(',')} L${controlPoints[0].join(',')} ${startEnd[1].join(',')}`;
    }
    case 'A': {
...
const getRawCommandStrings = (data) => data.replace(/[\s,]+/g, ' ')
  .match(/[A-Z\s\S]+?(?=[A-Z]|$)/gi)
